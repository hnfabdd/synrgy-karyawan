...
public class RegisterController {
    ...
    public ResponseEntity<Map> deleteRegisteredUser(@PathVariable(value = "username") String username) {
        User user = userRepository.checkExistingEmail(username);
        if (null == user) {
            return new ResponseEntity<Map>(templateCRUD.templateEror("User not found"), HttpStatus.OK);
        }

        // Perform deletion logic (e.g., userRepository.delete(user);)

        return new ResponseEntity<Map>(templateCRUD.templateSukses("User deleted successfully"), HttpStatus.OK);
    }*/
    ...
    public ResponseEntity<Map> saveRegisterByGoogle(@Valid @RequestBody RegisterModel objModel) throws RuntimeException {
        ...
        Map mapRegister = sendEmailRegister(objModel);
    ...
    public ResponseEntity<Map> saveRegisterManual(@PathVariable(value = "token") String tokenOtp) throws RuntimeException {
        ...
        //validasi jika sebelumnya sudah melakukan aktivasi
        if (user.isEnabled()) {
            return new ResponseEntity<Map>(templateCRUD.templateSukses("Akun lu sudah aktif, Silahkan lakukan login"), HttpStatus.OK);
        }
        ...
        return new ResponseEntity<Map>(templateCRUD.templateSukses("Sukses, Silahkan lakukan Login"), HttpStatus.OK);
    }

    /* belum work
    @DeleteMapping("/register-google/{username}")
    public ResponseEntity<Map> deleteGoogleRegisteredUser(@PathVariable(value = "username") String username) {
        User user = userRepository.checkExistingEmail(username);
        if (null == user) {
            return new ResponseEntity<Map>(templateCRUD.templateEror("User not found"), HttpStatus.OK);
        }
